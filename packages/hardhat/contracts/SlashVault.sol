 ...
 */
 ...
 interface ISlashToken {
     ...
     function fragmentToSlash(uint256 value) external view returns (uint256);
 
     function slashToFragment(uint256 slash) external view returns (uint256);
 ...
 contract SlashVault is Ownable, ReentrancyGuard {
     ...
     // SLASH
     ISlashToken public slash;
     // SLASH LP address
     IUniswapPair public slashLp;
     // Uniswap V2 Router
     IUniswapV2Router public router;
     // SLASH tokens reward per block.
     ...
     // The block number when SLASH mining starts.
     ...
     // Update the given pool's SLASH allocation point. Can only be called by the owner.
     function set(
         uint256 _pid,
         uint256 _allocPoint,
         bool _withUpdate
     ...
     // View function to see pending Slash on frontend.
         ...
         returns (uint256)
     {
         ...
         if (address(pool.lpToken) == address(slash)) {
             lpSupply = slash.balanceOfUnderlying(address(this));
         }
         if (block.number > pool.lastRewardBlock && lpSupply != 0) {
             uint256 slashReward = ((block.number - pool.lastRewardBlock) *
                 rewardPerBlock *
                 pool.allocPoint) / totalAllocPoint;
             accRewardPerShare += (slashReward * 1e12) / lpSupply;
         }
         return
     ...
     function updatePool(uint256 _pid) public {
         ...
         uint256 slashReward = ((block.number - pool.lastRewardBlock) *
             rewardPerBlock *
             pool.allocPoint) / totalAllocPoint;
         pool.accRewardPerShare += (slashReward * 1e12) / lpSupply;
         pool.lastRewardBlock = block.number;
     ...
     // Deposit tokens to SlashVault for SLASH allocation.
     function deposit(
         uint256 _pid,
         uint256 _amount,
         address _account
         ...
         if (address(pool.lpToken) == address(slash)) {
             _amount = slash.fragmentToSlash(_amount);
         }
     ...
     // Withdraw tokens from SlashVault.
     function withdraw(uint256 _pid, uint256 _amount) external {
         ...
         user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
         if (address(pool.lpToken) == address(slash)) {
             _amount = slash.slashToFragment(_amount);
         }
     ...
     function compoundBig(
         ...
         IERC20(address(slash)).safeTransferFrom(
             msg.sender,
             address(this),
             _amountTokenDesired
         );
         IERC20(address(slash)).approve(address(router), _amountTokenDesired);
         ...
         IERC20(address(slash)).safeTransfer(
             msg.sender,
